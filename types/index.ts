...
/* -------------------------------------------------------------------------- */
/*                       Agentic Operations Layer (U3)                        */
/* -------------------------------------------------------------------------- */

/** Operational recommendation actions (extensible; routing is a future seam). */
...
/** A deterministic operational recommendation with reasoning (never a bare label). */
...
/** Deterministic, explainable prioritization (computed, not stored). */
...
export interface TimelineEvent {
  ...
  at: string;
  detail?: string;
  source: "citizen" | "system" | "ai" | "admin";
}

/** A single recommended action within an AI operations brief. */
...
/**
 * Agentic Operations Brief (U3). Synthesizes deterministic signals (severity,
 * context, priority, recommendation) into an explainable operational picture.
 * Generated by Gemini and cached on the case; a deterministic fallback is used
 * when Gemini is unconfigured.
 */
export interface OperationsBrief {
  ...
  whyItMatters: string;
  recommendedPriority: SeverityLabel;
  nextActions: OperationsBriefAction[];
  dependencies: string[];
  risks: string[];
  /** 0-1 confidence. */
  confidence: number;
  /** Provenance: AI-generated vs deterministic fallback. */
...
export interface CivicCase {
  ...
  /** Cached agentic operations brief (U3) + its cache key (reportCount:status). */
  opsBrief?: OperationsBrief | null;
  opsBriefKey?: string | null;
